...
export function rapprocherDHL(t1, t2, id1, id2, fttc1, fttc2, tolfttc) {
    ...
    for (const prop in map1) {
        if (map2[prop] !== undefined) {
            ...
            if (Math.abs(map1[prop][fttc1]) === Math.abs(map2[prop][fttc2])) {
                ...
                delete map1[prop];
                delete map2[prop];
            } else if ((Math.abs(map1[prop][fttc1]) - Math.abs(map2[prop][fttc2])) < tolfttc) {
                map1[prop]['suspens'] = false;
                map1[prop]['difference'] = (Math.abs(map1[prop][fttc1]) - Math.abs(map2[prop][fttc2]));
                map1[prop]['comment'] = 'Différence négligeable frais';

                map2[prop]['suspens'] = false;
                map2[prop]['difference'] = (Math.abs(map1[prop][fttc1]) - Math.abs(map2[prop][fttc2]));
...
export function dhlXLS(result: any, workbook: Excel.Workbook, partnerheaders: string[], cashitheaders: string[],
    ...
    if (result.match.length > 0) {
        ...
        const worksheet1 = workbook.addWorksheet('Bons enregistrements DHL ');
    ...
    if (result.inexactmatch.length > 0) {
        ...
        const worksheet1 = workbook.addWorksheet('Suspends DHL Montant');
        worksheet1.columns = headers;
        worksheet1.getColumn(1).numFmt = 'dd/mm/yyyy';
        // worksheet1.getColumn(6).numFmt = 'dd/mm/yyyy';
    ...
    if (result.suspens1.length > 0 || result.suspens2.length > 0) {
        ...
        const worksheet1 = workbook.addWorksheet('Suspens DHL Référence');
        worksheet1.columns = headers;
        worksheet1.getColumn(1).numFmt = 'dd/mm/yyyy';
        // worksheet1.getColumn(6).numFmt = 'dd/mm/yyyy';
    ...
    if (partnerdata.length > 0) {
        const worksheet1 = workbook.addWorksheet('Données DHL Partenaires');
    ...
    if (cashitdata.length > 0) {
        const worksheet1 = workbook.addWorksheet('Données DHL Cash-IT');
