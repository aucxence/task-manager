...
export function rapprocherFT(t1, t2, id1, id2, nml1, nml2, fttc1, fttc2, tolnml, tolfttc) {
...
export function ftXLS(result: any, workbook: Excel.Workbook, partnerheaders: string[], cashitheaders: string[],
  ...
  if (result.match.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Bons enregistrements FT');
  ...
  if (result.inexactmatch.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Suspends FT Montant');
    worksheet1.columns = headers;
    worksheet1.getColumn(1).numFmt = 'dd/mm/yyyy';
    // worksheet1.getColumn(6).numFmt = 'dd/mm/yyyy';
  ...
  if (result.suspens1.length > 0 || result.suspens2.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Suspens FT Référence');
    worksheet1.columns = headers;
    worksheet1.getColumn(1).numFmt = 'dd/mm/yyyy';
    // worksheet1.getColumn(6).numFmt = 'dd/mm/yyyy';
  ...
  if (partnerdata.length > 0) {
    const worksheet1 = workbook.addWorksheet('Données FT Partenaires');
  ...
  if (cashitdata.length > 0) {
    const worksheet1 = workbook.addWorksheet('Données FT Cash-IT');
