...
export function rapprocherMG(t1, t2, id1, id2, nml1, nml2, fttc1, fttc2, tolnml, tolfttc) {
  ...
  for (const prop in map1) {
    if (map2[prop] !== undefined) {
      ...
      let mgfees = Math.round((Math.abs(+map1[prop][fttc1]) + Math.abs((+map1[prop]['ITComm']))) * 1.1925);
      if (map1[prop]['Tran Type'] === 'REC') { mgfees = 0; }
...
export function mgXLS(result: any, workbook: Excel.Workbook, partnerheaders: string[], cashitheaders: string[],
