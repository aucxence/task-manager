...
export function rapprocherRT(t1, t2, id1, id2, nml1, nml2, nml3, fttc1, fttc2, tolnml, tolfttc) {
  ...
  for (const prop in map1) {
    if (map2[prop] !== undefined) {
      ...
      let mgfees = Math.round((Math.abs(+map1[prop][fttc1])) * 1.1925);
...
export function rtXLS(result: any, workbook: Excel.Workbook, partnerheaders: string[], cashitheaders: string[],
  ...
  if (result.match.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Bons enregistrements RT');
  ...
  if (result.inexactmatch.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Suspends RT Montant');
    worksheet1.columns = headers;
    worksheet1.getColumn(1).numFmt = 'dd/mm/yyyy';
    // worksheet1.getColumn(6).numFmt = 'dd/mm/yyyy';
  ...
  if (result.suspens1.length > 0 || result.suspens2.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Suspens RT Référence');
    worksheet1.columns = headers;
    worksheet1.getColumn(1).numFmt = 'dd/mm/yyyy';
    // worksheet1.getColumn(6).numFmt = 'dd/mm/yyyy';
  ...
  if (partnerdata.length > 0) {
    const worksheet1 = workbook.addWorksheet('Données RT Partenaires');
  ...
  if (cashitdata.length > 0) {
    const worksheet1 = workbook.addWorksheet('Données RT Cash-IT');
