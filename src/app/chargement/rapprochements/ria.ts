...
export function rapprocherRIA(t1, t2, id1, id2, nml1, nml2, nml3, xfactor, fttc1, fttc2, tolnml, tolfttc) {
  ...
  for (const prop in map1) {
    if (map2[prop] !== undefined) {
      console.log('Nominal 1: ' + Math.abs(+map1[prop][nml1]) + ' = ' + Math.abs(map2[prop][nml2]));
      console.log('Nominal 2: ' + Math.abs(+map1[prop][nml3]) + ' = ' + Math.abs(map2[prop][nml2]));

      let mgfees = Math.round((Math.abs(+map1[prop][fttc1]) + (+map1[prop][xfactor])) * 1.1925);
      if (map1[prop]['Action'] === 'Paid') { mgfees = 0; }

      if ((Math.abs(+map1[prop][nml1]) === Math.abs(map2[prop][nml2]))
        ||
        (Math.abs(+map1[prop][nml3]) === Math.abs(map2[prop][nml2]))) {

        if (mgfees === Math.abs(map2[prop][fttc2])) {

          console.log('Frais: ' + mgfees + ' = ' + Math.abs(map2[prop][fttc2]) +
            ' = ' + (mgfees === Math.abs(map2[prop][fttc2])));
          ...
          console.log('Frais: ' + mgfees + ' = ' + Math.abs(map2[prop][fttc2]) +
            ' = ' + ((mgfees - Math.abs(map2[prop][fttc2])) < tolfttc));

          console.log(mgfees - Math.abs(map2[prop][fttc2]) + ' < ' + tolfttc);
        ...
        ||
        (Math.abs(Math.abs(+map1[prop][nml3]) - Math.abs(map2[prop][nml2])) < tolnml)) {

        map1[prop]['comment'] = 'Différence négligeable Nominal';
        map2[prop]['comment'] = 'Différence négligeable Nominal';

        map1[prop]['difference'] = (Math.abs(Math.abs(+map1[prop][nml1]) - Math.abs(map2[prop][nml2])) < tolnml) ?
          Math.abs(Math.abs(+map1[prop][nml1]) - Math.abs(map2[prop][nml2])) :
          Math.abs(Math.abs(+map1[prop][nml3]) - Math.abs(map2[prop][nml2]));

        map2[prop]['difference'] = (Math.abs(Math.abs(+map1[prop][nml1]) - Math.abs(map2[prop][nml2])) < tolnml) ?
          Math.abs(Math.abs(+map1[prop][nml1]) - Math.abs(map2[prop][nml2])) :
          Math.abs(Math.abs(+map1[prop][nml3]) - Math.abs(map2[prop][nml2]));
        ...
        map1[prop]['suspens'] = true;
        map1[prop]['difference'] = Math.min(
          Math.abs(+map1[prop][nml1]) - Math.abs(map2[prop][nml2]),
          Math.abs(+map1[prop][nml3]) - Math.abs(map2[prop][nml2]));
        map1[prop]['comment'] = 'Erreur sur le Nominal';

        map2[prop]['suspens'] = true;
        map2[prop]['difference'] = Math.min(
          Math.abs(+map1[prop][nml1]) - Math.abs(map2[prop][nml2]),
          Math.abs(+map1[prop][nml3]) - Math.abs(map2[prop][nml2]));
...
export function riaXLS(result: any, workbook: Excel.Workbook, partnerheaders: string[], cashitheaders: string[],
  ...
  if (result.match.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Bons enregistrements RIA');
  ...
  if (result.inexactmatch.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Suspends RIA Montant');
    worksheet1.columns = headers;
    worksheet1.getColumn(1).numFmt = 'dd/mm/yyyy';
    // worksheet1.getColumn(6).numFmt = 'dd/mm/yyyy';
  ...
  if (result.suspens1.length > 0 || result.suspens2.length > 0) {
    ...
    const worksheet1 = workbook.addWorksheet('Suspens RIA Référence');
    worksheet1.columns = headers;
    worksheet1.getColumn(1).numFmt = 'dd/mm/yyyy';
    // worksheet1.getColumn(6).numFmt = 'dd/mm/yyyy';
  ...
  if (partnerdata.length > 0) {
    const worksheet1 = workbook.addWorksheet('Données RIA Partenaires');
  ...
  if (cashitdata.length > 0) {
    const worksheet1 = workbook.addWorksheet('Données RIA Cash-IT');
